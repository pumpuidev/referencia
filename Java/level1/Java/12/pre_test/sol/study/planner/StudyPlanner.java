...
//////////////////-----------------
    ...
    public boolean isStudied(String bookName, int from, int to) throws StudyException {
		...
		for (int i = from; i <= to; ++i) {
            if (isStudied(bookName,i)) return true;
        }
		return false;
    }
	...
	private int learnablePagesCount(){
        Set<String> books = bookToPages.keySet();
		int pages = 0;
		try {
            for (String book : books) {
                pages += pageCountOf(book);
            }
		}
        catch (StudyException e) {
			// de ez úgysem fordulhat elő...
    ...
    public int compareTo(StudyPlanner that) {
		...
		return learnablePagesCount()-that.learnablePagesCount();
    }
}
